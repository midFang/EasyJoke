...
public abstract class CrashAppLog implements Thread.UncaughtExceptionHandler {
     ...
     * @param file   文件
     */
    protected abstract void sendCrashLogToServer(File folder, File file);
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    Looper.prepare();
                    Toast.makeText(mContext, "程序发生异常", Toast.LENGTH_SHORT).show();
                    Looper.loop();
                }
            }).start();
    ...
    public File getLogFile() {
        try {
            ...
            if (file.isDirectory()) {
                ...
                return files[0]; // 默认只存在一个 log 日志文件, 只删除第一个文件
